...
MODEL_ID_IDENTIFICA_METODO_ANALISE = os.getenv(
    "MODEL_ID_IDENTIFICA_METODO_ANALISE", DEFAULT_MODEL
)
...
llm_identifica_metodo_analise = prompt_metodo_analise | llm.with_structured_output(
    MetodoAnalise
)


def identifica_metodo_analise(
    state: State,
) -> Command[Literal["identifica_ticks", "chatbot_padrao"]]:
    """
    ...
    resposta_metodo_analise: MetodoAnalise = llm_identifica_metodo_analise.invoke(
        {"messages": messages}
    )  # type: ignore
    ...
    if "sem_analise" in resposta_metodo_analise.method_analysis:
        return Command(
            goto="chatbot_padrao",
            update={
                "mensagem_sistema": "Essa foi a pergunta feita pelo usuario:{messages}"
            },
        )
    return Command(
