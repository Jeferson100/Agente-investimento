...
class TratatandoDadosFundamentalistasComparacao:
    def __init__(self, tics: List[str], data_inicio: Optional[str] = None, data_fim: Optional[str] = None, colunas_drop: Optional[List[str]] = None) -> None:
        self.tics = tics
    ...
    async def dados_fundamentalistas(self, tic: str) -> pd.DataFrame:
        df = DadosFundamentalistas(tic=tic, data_inicio=self.data_inicio, data_fim=self.data_fim)
    ...
    async def coletando_dados_tickers(self):
        ...
        resutado_ticks = await asyncio.gather(*[self.transformer_loader(tic=tic) for tic in self.tics])
