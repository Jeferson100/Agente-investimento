...
class TratandoDadosValuationComparacao:
    ...
    async def preco_atual(self, ticker:str) -> float:
        ...
        preco = round(self.data_cache.get_history(await self.tratando_ticker(ticker))["Close"][-1], 2)
        return float(preco)

    async def indicadores_financeiros(self, ticker:str) -> Dict[str, float]:
        ind = IndicadoresFinanceirosAsync(ticker=await self.tratando_ticker(ticker=ticker))
        indicadores = await ind.todos_indicadores()
        return indicadores

    async def valuation_fluxo_caixa_descontado(self, ticker:str) -> tuple[pd.DataFrame, Dict[str, float]]:
    ...
    async def markdow_fluxo_caixa_descontado(self, ticker:str) -> tuple[str, Dict[str, float]]:
        fluxo_caixa, valuation_preco = await self.valuation_fluxo_caixa_descontado(ticker=ticker)
        markdow_fluxo = fluxo_caixa.to_markdown()
        return markdow_fluxo, {f"Preco do fluxo de caixa para {ticker}": valuation_preco["valor_por_acao"]}
    ...
    async def dados_valuation(self):
        ...
        fluxo_tasks = [self.markdow_fluxo_caixa_descontado(ticker) for ticker in self.tickers]
        indicadores_tasks = [self.markdow_indicadores_financeiros(ticker) for ticker in self.tickers]
        preco_tasks = [self.preco_atual(ticker) for ticker in self.tickers]
        
        gordon_results, fluxo_results, indicadores_results, preco_results = await asyncio.gather(
