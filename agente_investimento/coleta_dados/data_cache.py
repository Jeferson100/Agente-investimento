...
class DataCache:
    ...
    def get_historical_dez_anos(self, ticker_symbol):
        ...
        if cache_key not in self.history_cache_dez_anos:
            ticker = self.get_ticker(ticker_symbol)
            self.history_cache_dez_anos[cache_key] = ticker.history(period="10Y", interval="1d")
    ...
    def get_dowload(self, ticker_symbol):
        ...
        if ticker_symbol not in self.dowload_cache:
            self.dowload_cache[ticker_symbol] = yf.download(ticker_symbol, start="2004-01-01", end=datetime.today().strftime("%Y-%m-%d"))
    ...
    def get_financials(self, ticker_symbol):
        ...
        if ticker_symbol not in self.finances_cache:
            self.finances_cache[ticker_symbol] = yf.Ticker(ticker_symbol).get_financials()
    ...
    def get_balance_sheet(self, ticker_symbol):
        ...
        if ticker_symbol not in self.balance_sheet_cache:
            self.balance_sheet_cache[ticker_symbol] = yf.Ticker(ticker_symbol).get_balancesheet(freq="yearly")
    ...
    def get_quarterly_balance_sheet(self, ticker_symbol):
        if ticker_symbol not in self.quarterly_balance_sheet_cache:
            self.quarterly_balance_sheet_cache[ticker_symbol] = yf.Ticker(ticker_symbol).quarterly_balance_sheet
    ...
    def get_history_bovespa(self, start, end):
        ...
        if cache_key not in self.history_bovespa_cache:
            self.history_bovespa_cache[cache_key] = yf.download("^BVSP", start="2004-01-01", end=datetime.today().strftime("%Y-%m-%d"))
    ...
    def clear_history_cache(self, ticker_symbol=None):
        ...
        if ticker_symbol:
            keys_to_remove = [k for k in self.history_cache.keys() if k.startswith(ticker_symbol)]
