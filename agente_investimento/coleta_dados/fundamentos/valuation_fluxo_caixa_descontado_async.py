...
class ValuationFluxoCaixaDescontadoAsync:
    def __init__(
        self,
        receita_ano: float,
    ...
    async def calculo_perpetudidade(
    ...
    async def calcular_valuation(self) -> tuple[pd.DataFrame, Dict[str, float]]:
        ...
        for anos in range(1, self.anos_projecao + 1):
            (
                ...
                fluxo_caixa,
                valor_presente_fluxo,
            ) = await self.calcular_periodos(receita_ano=receita_ano, anos=anos)
