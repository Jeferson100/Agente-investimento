...
class DadosNoticiasBuscadorYahooAsync:
    ...
    async def beutfull_soup(self, navegador: webdriver.Chrome) -> Optional[BeautifulSoup]:
    ...
    async def get_text_async(self, element):
    ...
    async def pegando_titulo(self, soup: BeautifulSoup) -> List[str]:
        ...
        dados_titulo = await asyncio.gather(*[self.get_text_async(titulo) for titulo in titulos])
        return dados_titulo

    async def pegar_fonte(self, soup: BeautifulSoup) -> List[str]:
        fontes = soup.find_all(class_="s-source mr-5 cite-co")
        fonte_texto = await asyncio.gather(*[self.get_text_async(fonte) for fonte in fontes])
        return fonte_texto

    async def pegar_data(self, soup: BeautifulSoup) -> List[str]:
        tempos = soup.find_all(class_="fc-2nd s-time mr-8")
        tempo_texto = await asyncio.gather(*[self.get_text_async(tempo) for tempo in tempos])
        return tempo_texto
    ...
    async def process_page(self, soup: BeautifulSoup, page_num: int, dict_dados: Dict[str, List[str]]) -> None:
