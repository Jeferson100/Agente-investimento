...
class PassivoTotalMenosDivida:
    ...
    def passivo_nao_circulante(self) ->float:
        ...
        if isinstance(passivo_nao_circulante, pd.DataFrame):
            if 'TotalNonCurrentLiabilitiesNetMinorityInterest' in passivo_nao_circulante.index:
                out_terrenos = passivo_nao_circulante.loc['TotalNonCurrentLiabilitiesNetMinorityInterest'][0]
            else:
                out_terrenos = 0
