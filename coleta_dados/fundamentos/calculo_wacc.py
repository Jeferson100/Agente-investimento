...
class CalculoWACC:
    def __init__(self, ticker: str, start_date_retorno: str ="2004-01-01", end_date_retorno: str = datetime.today().strftime('%Y-%m-%d')):
        self.ticker = ticker
    ...
    def juros_livre(self) -> float:
        juros = (ip.timeseries("BMF12_SWAPDI36012")
    ...
    def retorno_mercado(self) -> float:
        ...
        ibov = yf.download('^BVSP', start=self.start_date_retorno, end=self.end_date_retorno)
    ...
    def calculo_divida(self) -> float:
        debt = self.valor_total_empresa() - self.valor_mercado() if self.valor_total_empresa() and self.valor_mercado() else 0
        if debt is None:
            raise ValueError("Erro: Não foi possível obter o valor total da empresa.")
        return float(debt)
    ...
    def custo_patrimonio(self) -> float:
        cost_of_equity = self.juros_livre() + self.beta_empresa() * self.retorno_mercado()
    ...
    def despesas_juros(self) -> float:
        ...
        if 'InterestExpense' not in df_financials.index:
            raise ValueError("Erro: A coluna 'InterestExpense' não está presente nos dados financeiros.")
    ...
    def total_divida(self) -> float:
        ...
        if total_debt is None:
            raise ValueError("Erro: Não foi 가능 obter o total da divida.")
        return float(total_debt)
    
    def custo_divida(self) -> float:
        cost_of_debt = (self.despesas_juros() / self.total_divida()) if self.total_divida() else self.juros_livre()
        return cost_of_debt
    ...
    def wacc(self) -> float:
        V = self.valor_mercado() + self.calculo_divida() if self.valor_mercado() and self.calculo_divida() else 1  # Evita divisão por zero

        wacc = (self.valor_mercado() / V * self.custo_patrimonio()) + (self.calculo_divida() / V * self.custo_divida() * (1 - self.custo_imposto()))
